...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: employees, count, error: employeesError }, { data: departments, error: departmentsError }] = await Promise.all([query, supabase.from("departments").select("id,name").eq("active", true).order("name")]);
  if (employeesError || departmentsError) throw new Error("Unable to load employee directory");
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="pt-5"><form className="grid gap-3 md:grid-cols-[1fr_180px_160px_140px_auto]"><label className="relative"><span className="sr-only">Search employees</span><Search className="absolute left-3 top-3 size-4 text-muted"/><input name="q" defaultValue={params.q} placeholder="Employee ID or designation" className="h-10 w-full rounded-xl border bg-[var(--surface)] pl-9 pr-3 text-sm"/></label><select name="department" aria-label="Department" defaultValue={params.department ?? ""} className="h-10 rounded-xl border bg-[var(--surface)] px-3 text-sm"><option value="">All departments</option>{(departments ?? []).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select><select name="status" aria-label="Employment status" defaultValue={params.status ?? ""} className="h-10 rounded-xl border bg-[var(--surface)] px-3 text-sm"><option value="">All statuses</option>{["active", "inactive", "probation", "resigned"].map(s => <option key={s}>{s}</option>)}</select><select name="sort" aria-label="Sort employees" defaultValue={params.sort ?? "name"} className="h-10 rounded-xl border bg-[var(--surface)] px-3 text-sm"><option value="name">Sort by ID</option><option value="joining">Newest joining</option></select><Button type="submit" variant="secondary"><ArrowUpDown className="size-4"/>Apply</Button></form></CardContent></Card>
    <Card className="overflow-hidden"><div className="overflow-x-auto" tabIndex={0} role="region" aria-label="Scrollable data table"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-[var(--surface-muted)] text-muted"><tr>{["Employee", "Department", "Designation", "Joined", "Status", "Actions"].map(h => <th key={h} className="px-5 py-3 text-xs font-semibold uppercase tracking-wide">{h}</th>)}</tr></thead><tbody className="divide-y">{(employees ?? []).map(employee => { const person = Array.isArray(employee.profile) ? employee.profile[0] : employee.profile; const department = Array.isArray(employee.department) ? employee.department[0] : employee.department; return <tr key={employee.id} className="hover:bg-[var(--surface-muted)]"><td className="px-5 py-4"><div className="flex items-center gap-3"><Avatar name={person?.full_name ?? "Employee"} src={person?.avatar_url} size="sm"/><div><p className="font-semibold">{person?.full_name}</p><p className="text-muted text-xs">{employee.employee_code} · {person?.email}</p></div></div></td><td className="px-5 py-4">{department?.name ?? "—"}</td><td className="px-5 py-4">{employee.designation}</td><td className="px-5 py-4">{format(new Date(`${employee.joining_date}T00:00:00`), "d MMM yyyy")}</td><td className="px-5 py-4"><Badge tone={statusTone[employee.employment_status as keyof typeof statusTone]}>{employee.employment_status}</Badge></td><td className="px-5 py-4"><div className="flex gap-2"><Button asChild size="sm" variant="outline"><Link href={`/employees/${employee.id}`}>View</Link></Button><ActionForm action={setEmploymentStatus} successMessage="Employee status updated."><input type="hidden" name="employeeId" value={employee.id}/><input type="hidden" name="status" value={employee.employment_status === "inactive" ? "active" : "inactive"}/><SubmitButton size="sm" variant="ghost" pendingLabel="Saving…">{employee.employment_status === "inactive" ? "Activate" : "Deactivate"}</SubmitButton></ActionForm></div></td></tr>; })}</tbody></table></div>
