...
import { businessDate, getOrganizationContext } from "@/lib/organization";
import { safeCallbackPath } from "@/lib/auth/callback-path";

export default async function EmployeeOverview() {
  const profile = await requireProfile(["employee"]); const supabase = await createClient();
  const [viewer, organization] = await Promise.all([getViewerContext(), getOrganizationContext()]);
  const today = businessDate(new Date(), organization.timezone);
  const weekStart = format(startOfWeek(parseISO(today), { weekStartsOn: 1 }), "yyyy-MM-dd");
  const employee = viewer?.employee;
  if (!employee) throw new Error("Employee profile is not available");
  const employeeId = employee?.id;
  const [attendanceRes, activeRes, weekRes, balancesRes, leaveRes, payrollRes, announcementRes, notificationRes, unreadRes] = employeeId ? await Promise.all([
    ...
    supabase.from("attendance_records").select("work_date,check_in,check_out,total_minutes,status").eq("employee_id", employeeId).not("check_in", "is", null).is("check_out", null).order("check_in", { ascending: false }).limit(1),
    ...
    supabase.from("notifications").select("id", { count: "exact", head: true }).eq("recipient_id", profile.id).eq("is_read", false),
  ]) : [{data:null},{data:[]},{data:[]},{data:[]},{count:0},{data:[]},{data:[]},{data:[]},{count:0}];
  if ([attendanceRes, activeRes, weekRes, balancesRes, leaveRes, payrollRes, announcementRes, notificationRes, unreadRes].some((result) => "error" in result && result.error)) throw new Error("Unable to load your dashboard");
  ...
  const chartData = Array.from({ length: 7 }, (_, index) => { const date = addDays(parseISO(weekStart), index); const row = week.find((item) => item.work_date === format(date,"yyyy-MM-dd")); return { day: format(date,"EEE"), hours: Number(((row?.total_minutes ?? 0)/60).toFixed(1)) }; });
  const hour = Number(new Intl.DateTimeFormat("en-GB", { timeZone: organization.timezone, hour: "2-digit", hourCycle: "h23" }).format(new Date()));
  const greeting = hour < 12 ? "Good morning" : hour < 17 ? "Good afternoon" : "Good evening";
  return <div className="space-y-6"><section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-muted text-sm">{format(parseISO(today), "EEEE, d MMMM yyyy")}</p><h2 className="mt-1 text-3xl font-bold tracking-tight">{greeting}, {profile.full_name.split(" ")[0]} <span aria-hidden>👋</span></h2><p className="text-muted mt-2">Here’s how your workday is flowing.</p></div><div className="flex items-center gap-3 rounded-2xl border bg-[var(--surface)] p-3"><Avatar name={profile.full_name} src={profile.avatar_url}/><div><p className="font-semibold">{employee?.designation ?? "Designation pending"}</p><p className="text-muted text-xs">{employee?.department ?? "Department unassigned"}</p><p className="text-muted text-xs">{employee?.employeeCode ?? "Employee ID pending"}</p></div></div></section>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      ["Weekly hours", `${Math.floor(weeklyMinutes/60)}h ${weeklyMinutes%60}m`, TrendingUp, "Across recorded workdays"], ["Attendance rate", week.length ? `${Math.round(presentDays/week.length*100)}%` : "—", CalendarCheck, `${presentDays} of ${week.length} recorded days`], ["Pending leave", String(leaveRes.count ?? 0), FileText, "Awaiting HR review"], ["Unread updates", String(unreadRes.count ?? 0), Bell, "Notifications awaiting you"],
    ...
    <div className="grid gap-4 lg:grid-cols-3"><AttendanceCard record={activeRes.data?.[0] ?? attendanceRes.data} serverNow={new Date().getTime()} timezone={organization.timezone}/><WeeklyHoursChart data={chartData}/></div>
    <div className="grid gap-4 lg:grid-cols-3"><Card><CardHeader><CardTitle>Leave balances</CardTitle><Button asChild variant="ghost" size="sm"><Link href="/leave">Manage <ArrowRight className="size-3"/></Link></Button></CardHeader><CardContent className="space-y-4">{(balancesRes.data ?? []).map((balance) => { const lt = Array.isArray(balance.leave_type) ? balance.leave_type[0] : balance.leave_type; const total=balance.balance_days+balance.used_days; return <div key={lt?.name}><div className="mb-1.5 flex justify-between text-sm"><span>{lt?.name}</span><strong>{balance.balance_days} days</strong></div><div className="h-2 rounded-full bg-[var(--surface-muted)]"><div className="h-full rounded-full bg-[var(--primary)]" style={{width:`${total ? balance.balance_days/total*100 : 0}%`}}/></div></div>})}{!balancesRes.data?.length && <p className="text-muted text-sm">No leave balances assigned.</p>}</CardContent></Card>
      <Card><CardHeader><CardTitle>Latest payroll</CardTitle><CircleDollarSign className="size-5 text-[var(--primary)]"/></CardHeader><CardContent>{payrollRes.data?.[0] ? <><p className="text-muted text-sm">{format(new Date(`${payrollRes.data[0].payroll_month}-01T00:00:00`),"MMMM yyyy")}</p><p className="mt-2 text-3xl font-bold">{formatCurrency(payrollRes.data[0].net_salary, organization.currency)}</p><Badge tone="success" className="mt-4">{payrollRes.data[0].status}</Badge><Button asChild variant="outline" className="mt-5 w-full"><Link href="/payroll">Open payslip</Link></Button></> : <p className="text-muted text-sm">No published payroll record yet.</p>}</CardContent></Card>
