...
import { isSupabaseConfigured, logSupabaseConfigurationError } from "@/lib/env";

export default function AuthLayout({ children }: { children: React.ReactNode }) { if (!isSupabaseConfigured) logSupabaseConfigurationError("auth layout"); return <main className="min-h-screen bg-[var(--surface)]">{!isSupabaseConfigured && <SetupNotice />}<div className="grid min-h-[calc(100vh-41px)] lg:grid-cols-[1.05fr_.95fr]">
