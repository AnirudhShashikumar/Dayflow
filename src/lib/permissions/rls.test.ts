...
describe("database isolation contract",()=>{
  ...
  it("guards employee payroll and documents",()=>{expect(sql).toContain("employee_id=current_employee_id() and status in('processed','paid')");expect(sql).toContain("employee_id=current_employee_id() and visibility='employee'")});
