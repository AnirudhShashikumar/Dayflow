...
export function Avatar({ name, src, size = "md" }: { name: string; src?: string | null; size?: "sm" | "md" | "lg" }) {
  ...
  if (src) {
    try { const url = new URL(src); safeSource = url.protocol === "https:" && url.hostname.endsWith(".supabase.co"); }
    catch { safeSource = false; }
  }
  if (src && safeSource) return <Image width={64} height={64} className={`${sizes[size]} rounded-full object-cover ring-2 ring-[var(--surface)]`} src={src} alt={`${name} avatar`} />;
