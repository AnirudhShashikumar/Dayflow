...
// HR/Admin nav — Attendance intentionally excluded per requirements
const managementNav = [
  ["Home", "/home", Home], ["Overview", "/hr", LayoutDashboard], ["Employees", "/employees", Users], ["Leave Requests", "/leave", FileText], ["Payroll", "/payroll", CircleDollarSign], ["Reports", "/reports", FileBarChart], ["Announcements", "/announcements", Megaphone], ["Activity Log", "/activity", Activity], ["Departments", "/departments", Building2], ["Settings", "/settings", Settings],
] as const;
...
export function AppShell({ profile, unreadCount, children }: { profile: Profile; unreadCount: number; children: React.ReactNode }) {
  ...
  // HR/Admin portal has a permanently visible, non-collapsible sidebar
  const isAdmin = profile.role === "hr" || profile.role === "admin";
  const [mobile, setMobile] = useState(false);
  // Collapse is only relevant for the employee portal; admin sidebar is always fully expanded
  ...
  // Admin sidebar is always fully expanded (never collapsed)
  const effectiveCollapsed = isAdmin ? false : collapsed;
  return <div className="min-h-screen bg-[var(--background)]">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex flex-col border-r bg-[var(--surface)] transition-[width,transform] duration-200 lg:translate-x-0", effectiveCollapsed ? "lg:w-[76px]" : "lg:w-64", mobile ? "w-72 translate-x-0" : "w-72 -translate-x-full")}>
      ...
      {!isAdmin && <Button variant="outline" size="icon" onClick={() => setCollapsed(!collapsed)} className="absolute -right-4 top-24 hidden size-8 rounded-full bg-[var(--surface)] lg:inline-flex" aria-label="Collapse sidebar">{collapsed ? <ChevronRight className="size-4"/> : <ChevronLeft className="size-4"/>}</Button>}
    </aside>
