...
create or replace function public.get_current_role() returns text language sql stable security definer set search_path=public as $$ select role from public.profiles where id=auth.uid() $$;
create or replace function public.is_management() returns boolean language sql stable security definer set search_path=public as $$ select coalesce(public.get_current_role() in ('hr','admin'),false) $$;
create or replace function public.is_admin() returns boolean language sql stable security definer set search_path=public as $$ select coalesce(public.get_current_role()='admin',false) $$;
create or replace function public.current_employee_id() returns uuid language sql stable security definer set search_path=public as $$ select id from public.employee_profiles where profile_id=auth.uid() $$;
...
begin if not public.is_management() then raise exception 'Permission denied'; end if; perform public.write_audit(p_action,p_entity_type,p_entity_id,p_summary,p_metadata); end $$;
...
begin
  if auth.uid() is not null and old.role<>new.role and not public.is_admin() then raise exception 'Only administrators can change roles'; end if;
  if auth.uid() is not null and not public.is_management() and (old.email<>new.email or old.full_name<>new.full_name or old.role<>new.role or old.account_status<>new.account_status) then raise exception 'Employees may only update their avatar'; end if;
  return new;
end $$;
...
begin
  if old is distinct from new then
    ...
    perform public.write_audit('employee.updated','employee',new.id,'Employee profile updated');
  end if;
  return new;
end $$;
...
begin
  ...
  perform public.write_audit('attendance.checked_in','attendance',result.id,'Employee checked in',jsonb_build_object('work_date',current_date)); return result;
end $$;
...
begin
  select * into result from attendance_records where employee_id=public.current_employee_id() and work_date=current_date for update;
  ...
  perform public.write_audit('attendance.checked_out','attendance',result.id,'Employee checked out',jsonb_build_object('total_minutes',mins)); return result;
end $$;
...
begin
  ...
  if requires then
    select balance_days into available from leave_balances where employee_id=emp and leave_type_id=p_leave_type_id and year=extract(year from p_start);
  ...
  perform public.write_audit('leave.submitted','leave_request',result.id,'Leave request submitted',jsonb_build_object('days',days)); return result;
end $$;
...
begin
  if not public.is_management() then raise exception 'Permission denied'; end if; if p_decision not in ('approved','rejected') then raise exception 'Invalid decision'; end if;
  ...
  perform public.write_audit('leave.'||p_decision,'leave_request',req.id,'Leave request '||p_decision,jsonb_build_object('comment',p_comment)); return result;
end $$;
...
declare result leave_requests; begin update leave_requests set status='cancelled' where id=p_request_id and employee_id=public.current_employee_id() and status='pending' returning * into result; if result.id is null then raise exception 'Only your pending request can be cancelled'; end if; insert into leave_request_history(leave_request_id,from_status,to_status,comment,actor_id) values(result.id,'pending','cancelled','Cancelled by employee',auth.uid()); insert into notifications(recipient_id,title,message,category,related_entity_type,related_entity_id,link) select id,'Leave request cancelled','An employee cancelled a pending leave request.','leave','leave_request',result.id,'/leave?id='||result.id from profiles where role in ('hr','admin') and account_status='active'; perform public.write_audit('leave.cancelled','leave_request',result.id,'Leave request cancelled'); return result; end $$;
...
create policy profiles_select on profiles for select using(id=auth.uid() or public.is_management());
create policy profiles_self_update on profiles for update using(id=auth.uid()) with check(id=auth.uid() and role=public.get_current_role());
create policy profiles_hr_update on profiles for update using(public.is_management()) with check(public.is_management() and (public.get_current_role()='admin' or role<>'admin'));
create policy departments_read on departments for select using(auth.uid() is not null);
create policy departments_admin_all on departments for all using(public.is_admin()) with check(public.is_admin());
create policy employees_read on employee_profiles for select using(profile_id=auth.uid() or public.is_management());
...
create policy employees_management_all on employee_profiles for all using(public.is_management()) with check(public.is_management());
create policy attendance_read on attendance_records for select using(employee_id=public.current_employee_id() or public.is_management());
create policy attendance_management_all on attendance_records for all using(public.is_management()) with check(public.is_management());
create policy leave_types_read on leave_types for select using(auth.uid() is not null);
create policy leave_types_management on leave_types for all using(public.is_management()) with check(public.is_management());
create policy balances_read on leave_balances for select using(employee_id=public.current_employee_id() or public.is_management());
create policy balances_management on leave_balances for all using(public.is_management()) with check(public.is_management());
create policy leave_requests_read on leave_requests for select using(employee_id=public.current_employee_id() or public.is_management());
create policy leave_requests_management on leave_requests for update using(public.is_management()) with check(public.is_management());
create policy leave_history_read on leave_request_history for select using(public.is_management() or exists(select 1 from leave_requests r where r.id=leave_request_id and r.employee_id=public.current_employee_id()));
create policy payroll_read on payroll_records for select using((employee_id=public.current_employee_id() and status in('processed','paid')) or public.is_management());
create policy payroll_management on payroll_records for all using(public.is_management()) with check(public.is_management());
create policy documents_read on employee_documents for select using((employee_id=public.current_employee_id() and visibility='employee') or public.is_management());
create policy documents_management on employee_documents for all using(public.is_management()) with check(public.is_management());
...
create policy notifications_management_insert on notifications for insert with check(public.is_management());
create policy announcements_read on announcements for select using(auth.uid() is not null and active and publish_date<=current_date and (expiry_date is null or expiry_date>=current_date) and (audience='all' or (audience='department' and audience_department_id=(select department_id from employee_profiles where profile_id=auth.uid())) or (audience='specific' and public.current_employee_id()=any(audience_employee_ids)) or public.is_management()));
create policy announcements_management on announcements for all using(public.is_management()) with check(public.is_management());
create policy activity_management_read on activity_logs for select using(public.is_management());
create policy org_settings_read on organization_settings for select using(auth.uid() is not null);
create policy org_settings_admin on organization_settings for all using(public.is_admin()) with check(public.is_admin());
...
create policy storage_employee_read on storage.objects for select using(bucket_id='employee-documents' and (public.is_management() or exists(select 1 from employee_documents d where d.file_path=name and d.employee_id=public.current_employee_id() and d.visibility='employee')));
create policy storage_management_insert on storage.objects for insert with check(bucket_id='employee-documents' and public.is_management());
create policy storage_management_update on storage.objects for update using(bucket_id='employee-documents' and public.is_management());
create policy storage_management_delete on storage.objects for delete using(bucket_id='employee-documents' and public.is_management());
